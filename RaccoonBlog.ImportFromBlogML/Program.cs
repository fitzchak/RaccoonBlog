    ...
    internal class Program {
        ...
        private static void ImportBlogPosts(IDocumentStore store, BlogMLBlog blog) {
            ...
            foreach (var post in blog.Posts) {
                string authorId;
                User user;
                if (post.Authors.Count > 0 && usersList.TryGetValue(post.Authors.Cast<BlogMLAuthorReference>().First().Ref, out user)) {
                    authorId = user.Id;
                } else {
                ...
                var ravenPost = new Post {
                    ...
                    Tags = post.Categories.Cast<BlogMLCategoryReference>().Select(x => blog.Categories.First(c => c.ID == x.Ref).Title).ToArray(),
                    AllowComments = true
                };
                ...
                commentsCollection.Comments = post.Comments.Cast<BlogMLComment>()
                    ...
                    .Select(
                        comment => new PostComments.Comment {
                            ...
                            Url = comment.UserUrl,
                            Important = usersList.Any(u => u.Value.FullName == comment.UserName),
                            //UserAgent = comment.,
